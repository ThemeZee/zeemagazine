...
function themezee_load_custom_css() { 
	...
	if ( !isset($options['themeZee_color_activate']) or $options['themeZee_color_activate'] != 'true' ) {
		...
		wp_register_style('zeeMagazine_colorscheme', $stylesheet, array('zeeMagazine_stylesheet'));
		wp_enqueue_style( 'zeeMagazine_colorscheme');
	}
}
...
function themezee_load_web_fonts() { 
	wp_register_style('themezee_default_font', 'http://fonts.googleapis.com/css?family=Cantarell');
